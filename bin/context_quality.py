...
--------------------------------------
  ...
  - STILL A PREFERENCE KNOB (no ground-truth labels): the 0.40/0.35/0.25 signal weights, the
    degradation sub-weights, the corrective lexicon, and the agent-note GATE (2026-07-01 it is
    grade-first: fires only when score < 80 AND occ >= 0.65; see band_of + CTX-MONITOR.md).
    Context windows (INVERTED 2026-07-31): the map lists the LEGACY 200k tier (Haiku, the 3.x
    family, Sonnet/Opus <= 4.5); everything else -- including every model newer than the map --
    defaults to 1M, the catalog norm since mid-2026. The old shape (list the 1M models, default
    200k) rotted twice the same way: a newly shipped model missed the list and occupancy read
    ~5x too full. The >window => 1M inference net still bumps a listed-200k model that provably
    carries more. Authoritative live source remains the Models API max_input_tokens; kept
...
#
# INVERTED 2026-07-31: the map now lists the 200k LEGACY tier and defaults the
# REST to 1M. The old shape (list the 1M models, default 200k) rotted twice, the
# same way both times: a newly shipped large-context model missed the list and
# was scored against 200k (~5x too small), so occupancy read ~5x too full and
# the nudge fired on near-empty sessions (first Opus 4.6/4.7 + Sonnet 4.6, when
# only opus-4-8 + fable-5 were listed; then the Claude 5 family -- claude-opus-5
# / claude-sonnet-5 matched nothing). Since mid-2026 every current Opus/Sonnet/
# Fable/Mythos ships 1M: NEW models are the common case and must be right BY
# DEFAULT; the finite, shrinking legacy set is what a static list can keep up
# with. The failure modes are asymmetric but both bounded: an unlisted LEGACY
...
# Version-style tags carry a DIGIT BOUNDARY (gate finding, 2026-07-31): a bare
...
def _model(rec):
    ...
    comparison identically to a dict -- so the scoreable-turn predicate is unchanged
    for every input. What changes is that detect_window can no longer be handed a
    dict, and the emitted "model" field can no longer be an object.
...
def _as_int(v):
    """Fail-open int coercion for transcript numbers (gate finding, 2026-08-01).
...
# CORROBORATED 2026-06-19 by the maintainers' compaction-anchor harvest over the
# corpus: well-sampled
...
def degradation_signal(records):
    ...
    # (Tightened 2026-06-18: the old "any re-read" proxy over-counted vs ContextQ.)
    ...
    # instructions, not corrections (review fix 2026-06-18).
...
def score_session(records, window=None):
    ...
    # the last turn, not the first. (review fix 2026-06-18)
...
# (the signal hook used to carry an inline copy "mirroring" this; a mirror enforced
# by nothing is how definitions drift). GRADE-FIRST GATE (2026-07-01, user pref "trust
# the grade"): a band fires ONLY when score < 80 (below grade A) AND occ >= 0.65.
#   score >= 80  OR  occ < 0.65  -> "ok"        (grade A+ or low fill => silent)
#   occ >= 0.75                  -> "strong"
#   0.65 <= occ < 0.75           -> "advisory"
# The old low-occupancy "quality" band is retired (occ < 0.65 now suppresses it).
# PREVIOUS behaviour + how to revert: CTX-MONITOR.md "Grade-first gate".
...
#
# COMPACTION POLICY (decision, 2026-08-01): records at or before the LAST
...
def compose_session(records, partial=False):
    ...
    for i, r in enumerate(records):
        ...
        if isinstance(content, str):
            ...
            # single-object shape. Its bytes used to fall through to the ENVELOPE --
            # documented as never riding in the window -- whenever anything else in
            # the record made `pieces` nonzero. It is unrecognized CONTENT.
            n = _blob_bytes(content)
            used += n
            pieces += 1
            _add(cats, "other", "unrecognized_blocks", n)
        ...
        if tur is not None:
            ...
            if result_blocks == 1:
                ...
                #
                # STRUCTURAL test, and MEASURED rather than assumed (2026-08-01, 40
